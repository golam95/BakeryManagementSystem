...
public class SettingEmployee extends JInternalFrame implements ActionListener, ItemListener {
	...
	public SettingEmployee(JFrame getParentFrame) {
		...
		String cat_empName = (String) Cat_Name.getSelectedItem();// here is the
																	// variabel
																	// for
																	// fill_upfrom
		...
		JLabel lbl_addemply = new JLabel("Add New Employee :");
		panel2.add(lbl_addemply, "RIGHT");
		...
		LblDA = new JLabel("Medical Allowance   :");
		LblHRA = new JLabel("Attendence Bonuse :");
		LblWA = new JLabel("Transaction Allowane  :");
		...
		chDA = new JCheckBox("Medical", false);
		chDA.addItemListener(this);
		chHRA = new JCheckBox("Attendence", false);
		chHRA.addItemListener(this);
		chWA = new JCheckBox("Transaction", false);
		...
		LblGPF = new JLabel("Absence Deduction :");
		LblIT = new JLabel("Tax Deduction:");
		LblGIS = new JLabel("Shelter Deduction :");
		LblPF = new JLabel("Loan Deductoin  :");
		LblLIC = new JLabel("Meal Deduction :");
		...
		chGPF = new JCheckBox("Absence", false);
		chGPF.addItemListener(this);
		chIT = new JCheckBox("Tax", false);
		chIT.addItemListener(this);
		chGIS = new JCheckBox("Shelter", false);
		chGIS.addItemListener(this);
		chPF = new JCheckBox("Loan", false);
		chPF.addItemListener(this);
		chLIC = new JCheckBox("Meal", false);
		...
		fill_form(cat_empName);// when we click than it value pass the other
								// frame
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (source == Cat_Name) {
			...
			String cat_empthirdName = (String) Cat_Name.getSelectedItem();
		...
		try {
			...
			if (!sCategory_Type.equals("") && !sCategory_Name.equals("") && !sBasic_Pay.equals("")
				...
				if (foundrec == 0) {
					...
					String temp = "INSERT INTO Settings VALUES (" + sCategory_Type + ",'" + sCategory_Name + "',"
							+ sBasic_Pay + ",'" + sDA + "','" + sHRA + "','" + sWA + "','" + sGPF + "','" + sIT + "','"
							+ sGIS + "','" + sPF + "','" + sLIC + "'," + sDA_Allow + "," + sHRA_Allow + "," + sWA_Allow
							+ "," + sGPF_Dedu + "," + sIT_Dedu + "," + sGIS_Dedu + "," + sPF_Dedu + "," + sLIC_Dedu
							+ ")";
	...
	public void edit_record() {
		...
		try {
...

			{
				...
				String temp = "UPDATE Settings SET " + "Category_type= " + sCategory_Type + ", Category_Name= '"
						+ sCategory_Name + "',Basic_Pay= " + sBasic_Pay + ", medical='" + sDA + "',attendence= '" + sHRA
						+ "',transaction= '" + sWA + "',absence = '" + sGPF + "',tax = '" + sIT + "',shelter = '" + sGIS
						+ "',loan = '" + sPF + "',meal = '" + sLIC + "',medical_Allow = " + sDA_Allow
						+ ",attendence_Allow = " + sHRA_Allow + ",transaction_Allow = " + sWA_Allow + ",absence_Dedu = "
						+ sGPF_Dedu + ",tax_Dedu = " + sIT_Dedu + ",shelter_Dedu = " + sGIS_Dedu + ",loan_Dedu = "
						+ sPF_Dedu + ",meal_Dedu = " + sLIC_Dedu + " WHERE Category_type= " + sCategory_Type;
				int result = stmt.executeUpdate(temp);
				if (result == 1) {
		...
		if (DResult == JOptionPane.YES_OPTION) {
			...
			try {
...

				{
					...
					String temp = "DELETE from Settings " + " WHERE Category_Name = '" + sCategory_Name + "'";
